...
当前工程尝试已完成 step 1、恢复后 step 2，各保存四份 rank checkpoint，
`engineering_audit.json` 的 14 项检查全部通过，包括恢复后参数变化、有限 loss、
权重同步、工具 token mask 和数据完整性。step 1 包含 32 条轨迹，
平均 response 长度约 14,866 token；采样 269 秒、梯度更新 65 秒、权重同步 3.6 秒，
总计约 382 秒。此为首次工程步的实测值，包含启动后的首次计算开销，不能作为稳定吞吐
或四卡相对单卡的加速倍数。恢复后的 step 2 约 240 秒；正式 50 步任务已进入
`pilot_50_steps` 阶段，尚不能据此视为 50 步完成。实时状态以日志、退出状态和审计文件为准。
