...
def obs_trend(observations: dict, region: str) -> dict:
    ...
    for pol, block in observations.items():
        ...
        if not series:
            continue
        last24, prev24 = _mean(series[-24:]), _mean(series[-48:-24])
        if last24 is None:
            continue
        rec = {"last24h_mean": last24}
...
def daily_signals(diagnostics: dict) -> dict:
    ...
    for d in days:
        ...
        blh = f.get("blh_max_m") or f.get("blh_m")
        ...
        if (f.get("rain_mm") or 0) >= 1:
            flags.append("rain")
        tmax = f.get("tmax_c")
        if tmax is not None and tmax >= 28 and (f.get("cloud_pct") is None or f["cloud_pct"] < 50):
...
def compute_assessment(observations: dict, diagnostics: dict, guidance: dict,
    ...
    trend = obs_trend(observations, region)
    result = {
